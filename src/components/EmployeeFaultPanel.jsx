  ...
  return (
    <div className={`p-4 rounded-xl border flex-1 transition-colors duration-300 ${containerBg}`}>
      ...
      <div className="overflow-x-auto">
          ...
          <tbody className={`${textPrimary} divide-y ${isDarkMode ? 'divide-slate-800/50' : 'divide-slate-100'}`}>
            {anomalies.map(pole => (
              <tr key={pole.id} className={isDarkMode ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}>
                <td className="py-3 text-rose-500 font-mono font-medium">{pole.id}</td>
                <td className="py-3">{pole.location}</td>
                <td className="py-3">{pole.uptime}%</td>
                <td className="py-3 text-right">
                  <span className="px-2 py-0.5 rounded font-medium text-[9px] border bg-rose-500/10 text-rose-500 border-rose-500/20 cursor-pointer">
                    Inspect
                  </span>
                </td>
              </tr>
        ...
        {anomalies.length === 0 && (
          <p className="text-center text-sm mt-8 text-emerald-500 font-medium">All nodes operating normally.</p>
        )}
      </div>
    </div>
  );
